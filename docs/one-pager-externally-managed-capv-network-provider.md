# One-Pager: A externally-managed CAPV Network Provider for the Supervisor Cluster
...
VKS today picks exactly one `NetworkProvider` implementation per Cluster (`vsphere-distributed`, `nsx-tier1`, `vpc`) and that provider drives cluster network provisioning, load balancer creation, VM interface construction, and provider-based webhook validation – none of which is appropriate for the Supervisor Cluster.

Rather than threading per-field opt-outs through every existing provider, this design introduces a **new network provider value, externally-managed**, that encodes the Supervisor Cluster’s "bootstrap stack owns the network, VKS just attaches VMs" contract end-to-end. Crucially, externally-managed is a VKS-internal notion, not a Supervisor platform notion. Its selection is assumed by VKS based on the CAPI Cluster CR being in a specifically tagged supervisor namespace (via the [clusters.kubernetes.vmware.com/supervisor-namespace](http://clusters.kubernetes.vmware.com/supervisor-namespace): "true" annotation). This explicit VKS-internal scoping is necessary to avoid confusion for other controllers (like vm-operator, mobility-operator, etc.).

## Requirements From Supervisor Cluster 2.0

The contract the externally-managed provider must implement, derived from Supervisor Cluster 2.0:
...
* Introduce a new value externally-managed for the per-Cluster network provider field `VSphereCluster.spec.network.provider`, which is the single source of truth for the per-Cluster network provider.
* Implement a externally-managed `NetworkProvider` in CAPV that satisfies the contract above: no network/LB provisioning, straight propagation of `VSphereMachine.spec.network.interfaces` into `VirtualMachine.spec.network.interfaces`, with dual-stack `ipamModes` derived from the referenced object when it is a `Subnet`.
* Gate selection of the externally-managed provider to the Supervisor’s system namespace via a new namespace annotation introduced for Supervisor 2.0; ordinary workload namespaces cannot pick it.
...
* Changing the behavior of `vsphere-distributed`, `nsx-tier1`, or `vpc` providers on workload Clusters.
...
* Re-introducing multi-network on `nsx-tier1`, portgroup-based `Network` inside a VPC namespace, or SR-IOV in a VPC namespace.

## Architecture Areas

This design builds on [One-Pager: Support VKS Cluster Transition from VDS / NSX T1 to NSX VPC Network](./one-pager-vks-cluster-transition.md), which introduced the per-Cluster network provider field `VSphereCluster.spec.network.provider` (the single source of truth for the per-Cluster network provider) with the values `vsphere-distributed`, `nsx-tier1`, and `vpc`. This design adds one new value to that set:
...
* Selecting the externally-managed network provider on `VSphereCluster.spec.network.provider`.
* Skipping provider-based validation in the GCC Cluster webhook.

Ordinary workload namespaces never carry this annotation, so the externally-managed provider and the validation skips it unlocks are unreachable from a workload Cluster.
...
* If the Cluster’s namespace carries `clusters.kubernetes.vmware.com/supervisor-namespace: "true"`, set `VSphereCluster.spec.network.provider` to externally-managed instead of the value that would otherwise be derived from the namespace’s network provider.
...
#### New externally-managed `NetworkProvider`

Add a new implementation of the existing [`NetworkProvider`](https://github.com/kubernetes-sigs/cluster-api-provider-vsphere/blob/main/pkg/services/interfaces.go) interface alongside `vsphere-distributed`, `nsx-tier1`, and `vpc`.
...
New rule:
* When `VSphereCluster.spec.network.provider == "externally-managed"`, the webhooks **skip provider-based validation** of `spec.network.interfaces` and run only schema-level checks (interface name uniqueness, valid name characters, `mtu` bounds, required fields).
...
```
provider == "externally-managed"  ->  externallyManagedNetworkProvider
```
...
If either dependency is off, the externally-managed provider is not registered and VKS falls back to the standard per-namespace or global network provider.
...
The externally-managed `NetworkProvider.ConfigureVirtualMachine` then straight- propagates this into the `VirtualMachine` spec, setting dual-stack `ipamModes` on eth0 because the referenced `Subnet` is dual-stack:
...
* **VKS** provides the per-Cluster network provider field `VSphereCluster.spec.network.provider` (single source of truth) plumbing this design extends with the externally-managed value.
...
## References: Existing Provider-Based Webhook Validation (Skipped for externally-managed)

For completeness, the validation paths that the externally-managed provider skips. Each item lists the webhook entry point and the provider-driven logic it runs today; under externally-managed only schema-level checks remain.
